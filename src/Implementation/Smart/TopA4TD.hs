...
topA4TD :: ( MonadState Context m
        , MonadReader (D.Domain p c) m
        , Handler (Render 186) d, Display d
        , LazyTransport t
        , KnownNat n
        , Flash f
        )
...
topA4TD transport' dinputs' vibro' touch' sht21' display' etc' = do
    ...
    leds         <- mkLeds frameBuffer [   0
                                        ...
                                        ,     15,     20, 21, 28,     33, 34, 41,     47, 48, 55
                                        ,     14,     19,     27,     32,     40,     46,     54
                                        , 10, 13,     18, 22, 26,     31, 35, 39,     45, 49, 53
                                        ,     12,     17,     25,     30,     38,     44,     52
                                        ,     11,     16, 23, 24,     29, 36, 37, 42, 43, 50, 51
                                       ] transport etc (replicate 62 true)

    ledsPerButton <- values "leds_per_button" [2, 2, 2, 2]
    ledsOfButton  <- matrix "leds_of_button"  [[0,1,0,0], [0,2,0,0], [3,5,0,0], [4,5,0,0]]
