...
topG4 transport' dinputs' vibro' touch' sht21' display' etc' = do
    ...
    leds           <- mkLeds frameBuffer [10, 11, 0, 1, 7, 6, 5, 4, 8, 9, 2, 3] transport etc (replicate 8 true <> replicate 4 false)
    ledsPerButton  <- values "leds_per_button" [2, 2, 2, 2]
    ledsOfButton   <- matrix "leds_of_button"  [[0,1,0,0], [2,3,0,0], [4,5,0,0], [6,7,0,0]]
