...
module Implementation.Smart.TopG2 where
...
topG2 :: ( MonadState Context m
         , MonadReader (D.Domain p c) m
         , Display (d 3), Handler (Render (Canvas1DSize 12)) (d 3)
         , LazyTransport t
         , Flash f
         , KnownNat n
         )
...
topG2 transport' dinputs' vibro' touch' sht21' display' etc' = do
    ...
    leds           <- mkLeds frameBuffer [8, 9, 2, 3, 7, 6, 5, 4, 10, 11, 0, 1] transport etc (replicate 4 true <> replicate 8 false)
    ledsPerButton  <- values "leds_per_button" [2, 2]
    ledsOfButton   <- matrix "leds_of_button"  [[0,1,0,0], [2,3,0,0]]
