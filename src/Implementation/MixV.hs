...
mix :: forall no ni nd nr m p c t.
...
onInit :: forall l ni no nd nr s t. (KnownNat ni, KnownNat nd, KnownNat no, KnownNat nr, KnownNat l) 
       => Mix ni nd no nr 
       -> Buffer l Uint8 
       -> Uint8 
...
onGetState :: (KnownNat ni, KnownNat nd, KnownNat no, KnownNat nr) 
           => Mix ni nd no nr
           -> Ivory eff ()
onGetState Mix{..} = do
...
onDo :: forall l ni no nd nr s t. (KnownNat l, KnownNat ni, KnownNat nd, KnownNat nr, KnownNat no) 
     => Mix ni nd no nr
     -> Buffer l Uint8 
     -> Uint8 
     -> Ivory (ProcEffects s t) ()
onDo Mix{..} buff size = do
    REL.onDo relays buff size


onDim :: (KnownNat l, KnownNat nd)
      => Mix ni nd no nr
      -> Buffer l Uint8 
      -> Uint8 
      -> Ivory (ProcEffects s t) ()
