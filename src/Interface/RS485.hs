...
data RS485 rn tn where
    RS485 :: (UART (u rn tn), Output o, KnownNat rn, KnownNat tn)
          => { uart  :: u rn tn
             , rede  :: o
             }
            -> RS485 rn tn
...
rs485 :: ( MonadState Context m, MonadReader (D.Domain p c) m
         , UART (u rn tn), Show (u rn tn)
         , Output o, Pull p d
         , KnownNat rn, KnownNat tn
         )
      => (p -> m (u rn tn)) -> (p -> d -> m o) -> m (RS485 rn tn)
rs485 uart' rede' = do
    mcu' <- asks D.mcu
...
receive :: RS485 rn tn -> (Uint16 -> Ivory (ProcEffects s t) ()) -> Ivory (ProcEffects s t) ()
receive RS485{..} = U.receive uart



transmit :: RS485 rn tn
...
configureRS485 :: RS485 rn tn -> Uint32 -> WordLength -> StopBit -> Parity -> Ivory eff ()
