...
mkNetif :: (MonadState Context m, MonadReader (D.Domain p c) m, Enet e, LwipPort e)
        => (p -> m e) -> m (Record NETIF_STRUCT)
mkNetif enet' = do
    mcu             <- asks D.mcu
    let mac          = I.mac mcu
    ...
    let netifStatusCallback = proc "netif_callback" $ \netif -> body $ do
            ...
            when (flags' .& netif_flag_up /=?  0) $ call_ netifOnUpCallback
...
addNetifOnUpCallback :: MonadState Context m => (forall s. Ivory (ProcEffects s ()) ()) -> m ()
addNetifOnUpCallback = addBody "netif_on_up_callback"

rxTask :: (LwipPort e, Enet e) => e -> NETIF s1 ->  Ivory (ProcEffects s2 ()) ()
