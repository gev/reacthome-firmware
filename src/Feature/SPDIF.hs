...
mkSpdif :: (MonadState Context m, KnownNat n,
            Handler HandleI2SRX i)
            => i -> m (SPDIF n)
mkSpdif i2s = do
    
    let name = "spdif"
    i2sSpdifQueue  <- queue (name <> "_i2s_spdif_queue") =<< records_ (name <> "_i2s_buff_spdif")
    i2sSpdifSample <- record (name <> "_i2s_spdif_sample") [left .= izero, right .= izero]
