...
import           Protocol.OneWire.Master.Search
...
ds18b20 ow od = do
    ...
    s    <- mkSearch        onewire (onData ds) (onError ds)


    addTask $ delay      10000     (name <> "_search") $ search s
...
onData :: DS18B20 -> Uint8 -> Buffer 8 Uint8 -> Ivory (ProcEffects s ()) ()
onData _ _ _ = pure ()
